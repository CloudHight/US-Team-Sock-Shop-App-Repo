pipeline {
  ...
  stages {
    ...
    stage ('Slack Notification for stage dep') {
      steps {
        slackSend channel: 'Cloudhight', message: 'New Stage Deployment', teamDomain: '21st-april-sock-shop-project-us-team', tokenCredentialId: 'slack'
      }
    }
    ...
    stage ('DAST Scan') {
      steps {
        sh '''
          chmod 777 $(pwd)
          docker run -v $(pwd):/zap/wrk/:rw -t ghcr.io/zaproxy/zaproxy:stable zap-baseline.py -t https://stage.wahd.solutions -g gen.conf -r testreport.html || true
        '''
      }
    }
    ...
    stage ('Slack Notification for prod dep') {
      steps {
        slackSend channel: 'Cloudhight', message: 'New Production Deployment', teamDomain: '21st-april-sock-shop-project-us-team', tokenCredentialId: 'slack'
      }
    }
  }
}
