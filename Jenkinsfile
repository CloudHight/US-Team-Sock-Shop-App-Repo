pipeline {
  ...
  stages {
    ...
    stage ('Slack Notification for prod') {
      steps {
        slackSend channel: 'Cloudhight', message: 'New Stage Deployment', teamDomain: '8th-sept-2025-sock-shop-e-commerce-project-eu-team2', tokenCredentialId: 'slack'
      }
    }
    ...
    stage ('DAST Scan') {
      steps {
        sh '''
          chmod 777 $(pwd)
          docker run -v $(pwd):/zap/wrk/:rw -t ghcr.io/zaproxy/zaproxy:stable zap-baseline.py -t https://stage.set30.space -g gen.conf -r testreport.html || true
        '''
      }
    }
    ...
    stage ('Slack Notification') {
      steps {
        slackSend channel: 'Cloudhight', message: 'New Production Deployment', teamDomain: '8th-sept-2025-sock-shop-e-commerce-project-eu-team2', tokenCredentialId: 'slack'
      }
    }
  }
}
